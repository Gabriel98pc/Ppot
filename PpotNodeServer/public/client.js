...
window.addEventListener('load', function () {
	...
	socket.on('LOGIN_SUCCESS', function (id, name, coins, password) {
		var currentUserAtr = id + ':' + name + ':' + coins + ':' + password;
	...
	socket.on('UPDATE_CURRENT_BET_AMOUNT', function (userId, newBetAmount) {
		var currentUserAtr = userId + ':' + newBetAmount;

		if (window.unityInstance != null) {
			window.unityInstance.SendMessage('NetworkManager', 'OnUserUpdatedBetAmount', currentUserAtr);
		}
	...
	socket.on('SPAWN_PLAYER', function (id, name, currentBet) {

		var currentUserAtr = id + ':' + name + ':' + currentBet;
